import 'package:khm_app/src/core/network/dio_network.dart';
import 'package:khm_app/src/core/utils/injections.dart';
import 'package:khm_app/src/features/user/data/data_sources/remote/abstract_article_api.dart';
import 'package:khm_app/src/features/articles/data/data_sources/remote/articles_impl_api.dart';
import 'package:khm_app/src/features/articles/domain/repositories/abstract_articles_repository.dart';
import 'data/data_sources/local/user.dart';
import 'data/repositories/auth_repo_impl.dart';
import 'domain/usecases/articles_usecase.dart';

initUserInjections() {
  // sl.registerSingleton<ArticlesImplApi>(ArticlesImplApi(DioNetwork.appAPI));
  // sl.registerSingleton<AbstractArticlesRepository>(UserRepositoryImpl(sl()));
  sl.registerSingleton<UserSharedPrefs>(UserSharedPrefs(sl()));
  // sl.registerSingleton<ArticlesUseCase>(ArticlesUseCase(sl()));
}
